  ...
  _createClass(Dice, [{
    ...
    value: function reroll(values) {
      ...
      do {
        ...
        var rerolled = additionalRolls > 0;
    ...
    value: function drop(count) {
      ...
      var lowestIndecies = [];
      this.rolledDice.forEach(function (die, index) {
        if (die.invalid || count.value === 0) {
          return;
        }

        for (var i = 0; i < count.value; i++) {
          var lowestIndex = lowestIndecies[i];
          if (lowestIndex === undefined || die.value < _this.rolledDice[lowestIndex]) {
      ...
      lowestIndecies.forEach(function (dieIndex) {
        _this.rolledDice[dieIndex].Invalidate(InvalidReasons.DROPPED);
      });
    }
  }, {
    key: 'keep',
